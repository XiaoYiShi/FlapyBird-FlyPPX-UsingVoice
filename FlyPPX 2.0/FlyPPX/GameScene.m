...
- (id)initWithSize:(CGSize)size
{
    if (self = [super initWithSize:size]) {
        ...
        //音量
        _voiceLabel = [[SKLabelNode alloc] initWithFontNamed:@"Helvetica"];
        [_voiceLabel setPosition:CGPointMake(self.size.width/2 + 100, self.size.height-50)];
//        [_voiceLabel setText:[NSString stringWithFormat:@"%@", [NSNumber numberWithInteger:_power]]];
        [self addChild:_voiceLabel];
...
- (void)updateMeters {
    ...
    //获取瞬时音量
    _power = [self.recorder peakPowerForChannel:0];
    ...
    if (_power > -35) {
        ...
        if (_player.physicsBody.velocity.dy == 300) {
...
- (AVAudioRecorder *)recorder{
    if (_recorder == nil) {
        //生成的录音的存放路径
        NSString *path = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES).lastObject stringByAppendingPathComponent:@"123.caf"];
        NSLog(@"%@",path);
